...
**STATUS: DONE (implemented 2026-07-10).** Implemented as **signed raw-x86 `.em`** per the audit's recommended primary fix: a v4 `.em` format (v3 layout byte-identical + an additive Ed25519 signature block) verified before `alloc_executable_rw`.

> **CORRECTION NOTE (2026-07-10, post-audit): the F2 secondary (IL-`.em` re-lowering) is NO LONGER deferred.** The thin three-address IR shipped as Stage A (2026-07-10, `COMPILER_PIPELINE.md` §5's Stage-A note + `CODEGEN_SPEC.md` §5's Stage-A note — `src/thin_ir.{hpp,cpp}` / `src/thin_lower.{hpp,cpp}` / `src/thin_emit.{hpp,cpp}`), and the **v5 IR `.em` serialization shipped as Stage B** (the v5 `.em` format, `src/em_file.hpp` `EM_VERSION_V5` + the v5 per-function record's `is_ir` byte + `ir_blob` record; the `ir_blob` codec is `src/thin_ir_ser.{hpp,cpp}`). The loader deserializes + validates + re-emits the IR to x64 via `emit_x64` BEFORE `alloc_executable_rw` — the exact re-emit-at-load security model the audit's (d)/(e) "secondary, defense-in-depth" option described. See `BUNDLING_AND_EM_MODULES.md` §2.5.2 (the v5 format + the re-emit-at-load security model). The "No." answer in subsection (c) below and the "F2 secondary remains deferred" clause in this STATUS block are **stale** — preserved verbatim as the audit's original record, but superseded by the Stage A/B shipment. The cross-check at the end of this file ("Accurate. (Relevant to F2's finding that no serializable IR exists today.)") is likewise superseded: a serializable IR now exists.

 **Crypto choice (the audit's (a) vs (b)):** vendored a small Ed25519 (orlp/ed25519, public domain, ~1k LoC) under `thirdparty/ed25519/` — NOT Windows CNG — so ember stays dependency-free and portable (the audit leaned toward (a); CNG would couple ember to Windows and the build uses no crypto lib). `seed.c`'s CSPRNG (wincrypt on Windows) is excluded via `-DED25519_NO_SEED` for portability + so the test harness uses a DETERMINISTIC seed (a tamper test must be reproducible); production hosts that want a CSPRNG seed provide their own 32-byte seed to `ember::ed25519::keypair_from_seed`. **Signature-scheme note:** standard Ed25519 (RFC 8032) signs the message bytes directly (PureEdDSA) and uses SHA-512 as its INTERNAL hash; there is no separate SHA-256 pass (the audit's "Ed25519 over SHA-256" is read as "cryptographic content authentication via Ed25519 over the .em content" — a real hash, not the FNV1a identity hash; the scheme's internal hash is SHA-512 per the standard). **Format:** v4 = v3 content (header → name directory) + a 104-byte signature block (`sig_magic` "EMSG" | `payload_len` | `pubkey_id`[32] | `signature`[64]); the signed payload is the v3 content bytes; the loader cross-checks `payload_len` == end of name dir, verifies Ed25519 BEFORE `alloc_executable_rw`, and rejects on mismatch (no exec page published). v1/v2/v3 keep the "trailing bytes == 0" check (no signature block); the loader accepts v1/v2/v3/v4. `write_em_file` still emits UNSIGNED v3 (the dev/round-trip path — all existing tests unchanged); new `write_em_file_signed` emits v4. **Key management (secure-boot-style opt-in, the honest minimal v1):** `EmVerifyPolicy{ vector<array<uint8_t,32>> trusted_keys; }` — empty keyring (or null) = DEV MODE (unsigned v1/v2/v3 accepted; v4 rejected with a clear error — a v4 module IS signed, so running it unverified is worse than honest unsigned dev code); non-empty keyring = SIGNED-ONLY (v1/v2/v3 rejected; v4 loads only if its signature verifies against a trusted key). The signing key stays OFF the host; the host gets only verification public keys. CLI flag `--verify-em-key <path>` (repeatable) reads a 32-byte pubkey file and opts into signed-only mode for both `--load-em` and `link` directives. **Regression:** `em_signed_test` (ctest) cases A-E + the `build_em_exports` `format_version >= EM_VERSION_V2` fix (the EM_VERSION bump from 3→4 would have made v3 modules report `unknown_sig` without it). **Gate:** ctest 25/25 (was 24, +1 `em_signed`), lang 268/0/0. F2 secondary (IL-`.em` re-lowering) remains deferred until the SSA-lite IR ships AND a concrete threat justifies the larger loader surface. See the F2 commit (`6f333aa`) message for the full decision + the BUNDLING §2.5.1 / SAFETY §1 doc updates that land with it.
...
(c) **Does ember have a serializable IR today?** ~~No.~~ **CORRECTION (2026-07-10): YES — the thin three-address IR shipped as Stage A, and the v5 IR `.em` serialization shipped as Stage B.** The original audit answer below is preserved verbatim as the historical record; it is **stale**.

> **Original audit answer (stale, superseded):** `src/codegen.hpp` line 1-2: "codegen - tree-walking AST -> x86-64… a simple stack-spilling tree-walker (correctness first)." `src/codegen.cpp` `struct CG` walks the AST directly and emits x64; ~~there is no `IrFunction`/`IrInstr`/`IrValue`/`BasicBlock`/`run_linear_scan`/`emit_x64` in the codegen source (verified by grep).~~ **(Now stale: `src/thin_ir.{hpp,cpp}` ship `ThinFunction`/`ThinBlock`/`ThinInstr` + the stable `ThinOp` enum; `src/thin_lower.{hpp,cpp}` ship `lower_function`; `src/thin_emit.{hpp,cpp}` ship `emit_x64`.)** `docs/spec/CODEGEN_SPEC.md` §5 (line 238) and `docs/spec/COMPILER_PIPELINE.md` §5 both ~~mark "SSA-lite" IR as **deferred**~~ **(now carry a Stage-A note: the thin three-address IR — a deliberate subset of the SSA-lite design — is implemented behind `enable_ir_backend`.)** So an "IL-`.em`" ~~would require a **new serializable IR pass between sema and codegen** — it does not exist today.~~ **(Now exists: the v5 `.em` format carries the `ThinFunction` IR via the `ir_blob` record; the loader re-emits via `emit_x64`. See `BUNDLING_AND_EM_MODULES.md` §2.5.2.)**
