...
| `../spec/TYPE_SYSTEM.md` | Primitives, struct layout (host-mapped vs script-declared), fixed arrays, slices, the no-raw-pointer rule, implicit/explicit conversion matrix, operator result types, the unsigned-index rule that makes bounds-checking a runtime-only (not signedness) concern, `void` return-path enforcement, `auto`'s narrow scope, annotation argument grammar. |
| `../spec/COMPILER_PIPELINE.md` | Lexer token set, grammar, AST/sema, shipped tree-walking lowering, error model, and clearly deferred SSA-lite/regalloc design. |
| `../spec/CODEGEN_SPEC.md` | Win64 calling convention, frame/code emission, exact encoder subset, fixups, calls, bounds/div traps, and the deferred linear-scan design clearly labeled. |
| `../spec/BINDING_API.md` | Shipped `BindingBuilder`/`NativeSig`, script-type→Win64 mapping and slice convention; fluent builders/descriptors are labeled deferred. |
| `../spec/SAFETY_AND_SANDBOX.md` | Threat/trust model stated explicitly. Instruction budget mechanism (where counters live, what trap-on-exhaustion does), stack depth guard, bounds checking policy, `PERM_FFI` compile-time gating, the single non-local-unwind primitive that all traps funnel through, what is explicitly *not* checked (documented gaps, not oversights). |
...
- **v0.1** - hand-built IR for `fn add(i64,i64)->i64`, direct x64
  ...
  acceptance criteria in `../spec/CODEGEN_SPEC.md` Section 14 (int path, float path,
...
- **v0.2** ✓ shipped - the full frontend landed: lexer, recursive-descent
  ...
  SSA-lite IR + linear-scan regalloc specified in `../spec/COMPILER_PIPELINE.md`
...
- **v0.3** - the binding-correctness milestone. Three pieces:
  ...
  `../spec/BINDING_API.md` Section 4, now test-proven not just spec-asserted);
  ...
  (c) the `../spec/BINDING_API.md` truth-fix annotating that the fluent
...
- **v0.4** ✓ shipped (first hardening + safe execution) - two commits.
  ...
  `../spec/SAFETY_AND_SANDBOX.md` §2-§4 — `context_t` (setjmp/`__builtin_longjmp`
...
- **v1.0** - stable native binding API (the fluent `TypeBuilder`/
  ...
  **Concurrency + Tier 2 batch shipped v1.0 (commit e5d1814 + follow-ons):**
  ...
  - **Context thread-safety (Option D + B1)** — `context_t` restructured
    ...
    safe; legacy pointer kept as a null fallback). See `../spec/SAFETY_AND_SANDBOX.md`
    §8a, `plan_CONTEXT_THREADSAFETY.md`.
  ...
  - **Sync queue primitives (`extensions/sync/`)** — atomics
    ...
    contention). See `plan_SYNC_QUEUES.md`.
...
- No parser generator - grammar is small, hand-rolled recursive
  descent (`../spec/COMPILER_PIPELINE.md` Section 2).
...
- No phi nodes in IR - slot-backed merge vars instead
  (`../spec/COMPILER_PIPELINE.md` Section 5, `../spec/CODEGEN_SPEC.md` Section 5).
