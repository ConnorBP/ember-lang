...
EmberPreserved SimplifyCFGPass::run(ThinFunction& f, EmberAnalysisManager&) {
    ...
    while (iter_changed) {
        ...
        // Resolve constants defined in the same block. This covers a branch
        // directly driven by ConstBool/ConstInt and lowering's common shape:
        // ConstBool/ConstInt, compare with zero, Branch. Tracking is local so
        // mutable, non-SSA VRegs cannot leak an invalid fact across a join.
        for (auto& blk : f.blocks) {
            std::unordered_map<VReg, int64_t> constants;
            for (const auto& in : blk.instrs) {
                if (in.op == ThinOp::ConstInt || in.op == ThinOp::ConstBool) {
                    if (in.dst) constants[in.dst] = in.imm.i;
                    continue;
                }
                if (in.op == ThinOp::Move) {
                    auto src = constants.find(in.src1);
                    if (in.dst) {
                        if (src != constants.end()) constants[in.dst] = src->second;
                        else constants.erase(in.dst);
                    }
                    continue;
                }
                if (in.op == ThinOp::Cmp) {
                    auto lhs = constants.find(in.src1);
                    const bool rhs_is_imm = in.src2 == 0;
                    auto rhs = constants.find(in.src2);
                    if (in.dst && lhs != constants.end() &&
                        (rhs_is_imm || rhs != constants.end())) {
                        const int64_t a = lhs->second;
                        const int64_t b = rhs_is_imm ? in.imm.i : rhs->second;
                        bool result = false;
                        if (in.meta.is_unsigned) {
                            const uint64_t ua = static_cast<uint64_t>(a);
                            const uint64_t ub = static_cast<uint64_t>(b);
                            switch (in.meta.cmp) {
                            case 0: result = ua == ub; break;
                            case 1: result = ua != ub; break;
                            case 2: result = ua <  ub; break;
                            case 3: result = ua <= ub; break;
                            case 4: result = ua >  ub; break;
                            case 5: result = ua >= ub; break;
                            default: constants.erase(in.dst); continue;
                            }
                        } else {
                            switch (in.meta.cmp) {
                        ...
                        constants[in.dst] = result ? 1 : 0;
                    } else if (in.dst) {
                        constants.erase(in.dst);
                    }
                    continue;
                }
                if (in.dst) constants.erase(in.dst);
            }

            if (blk.term.kind != TermKind::Branch) continue;
            auto cond = constants.find(blk.term.cond);
            if (cond == constants.end() && blk.term.target != blk.term.false_target)
                continue;

            const uint32_t taken = blk.term.target == blk.term.false_target
                ? blk.term.target
                : (cond->second != 0 ? blk.term.target : blk.term.false_target);
            ThinTerm jmp;
        ...
        // Mark entry-reachable blocks using terminator edges, remove every
        // other block, then compact IDs because thin_emit indexes labels by ID.
        ...
        while (!work.empty()) {
            ...
            auto visit = [&](uint32_t id) {
                ...
                if (it != id_to_index.end() && !reachable[it->second]) {
        ...
        for (uint8_t r : reachable) has_unreachable |= r == 0;
        if (has_unreachable) {
        ...
        auto compact_ids = [&]() {
            ...
            for (size_t i = 0; i < f.blocks.size(); ++i) {
                ...
                if (blk.term.kind == TermKind::Jmp) {
                    ...
                    if (target != remap.end()) blk.term.target = target->second;
        ...
        // Merge one A -> B edge per iteration. Recomputing predecessors after
        // each merge keeps the sole-predecessor test exact and also exposes
        // newly trivial chains to the next fixpoint round.
        std::vector<std::unordered_set<uint32_t>> predecessors(f.blocks.size());
        for (const auto& blk : f.blocks) {
            auto add_pred = [&](uint32_t target) {
                if (target < predecessors.size())
                    predecessors[target].insert(blk.id);
            };
            if (blk.term.kind == TermKind::Jmp) {
                add_pred(blk.term.target);
            } else if (blk.term.kind == TermKind::Branch) {
                add_pred(blk.term.target);
                add_pred(blk.term.false_target);
            }
        }
        ...
        for (size_t ai = 0; ai < f.blocks.size(); ++ai) {
            ...
            const uint32_t bid = a.term.target;
            if (bid == 0 || bid == a.id || bid >= f.blocks.size()) continue;
            if (predecessors[bid].size() != 1 ||
                !predecessors[bid].count(a.id)) continue;

            ThinBlock& b = f.blocks[bid];
            a.instrs.insert(a.instrs.end(),
                            std::make_move_iterator(b.instrs.begin()),
                            std::make_move_iterator(b.instrs.end()));
            a.term = b.term;
            f.blocks.erase(f.blocks.begin() + static_cast<ptrdiff_t>(bid));
            compact_ids();
            merged = true;
            iter_changed = true;
            changed = true;
