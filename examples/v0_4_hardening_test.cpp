...
static ember::TrapReason run_under_safetrap(const std::string& src, int64_t budget = 100'000'000) {
    ...
    if (EMBER_SETJMP(ectx.checkpoint)) { return ectx.last_trap; }  // trap fired
...
static bool counter_wrap_guards_test() {
    ...
    if (EMBER_SETJMP(ectx.checkpoint)) budget_trap=ectx.last_trap==TrapReason::BudgetExceeded;
    ...
    if (EMBER_SETJMP(ectx.checkpoint)) huge_budget_ok=false;
    ...
    if (EMBER_SETJMP(ectx.checkpoint)) depth_trap=ectx.last_trap==TrapReason::StackOverflow;
    else (void)ember_call_void(main_entry,&ectx);
    ectx.has_checkpoint=false;
...
static bool native_reentry_depth_test() {
    ...
    int64_t recovered=-1;
    if (EMBER_SETJMP(ectx.checkpoint) == 0) {
...
static bool budget_300_native_calls_trap() {
    ...
    ectx.has_checkpoint=true;
    bool trapped=false;
    if (EMBER_SETJMP(ectx.checkpoint)) trapped = (ectx.last_trap==ember::TrapReason::BudgetExceeded);
...
static bool b1_per_context_max_call_depth() {
    ...
    if (EMBER_SETJMP(ectxA.checkpoint)) a_trapped=(ectxA.last_trap==ember::TrapReason::StackOverflow);
    ...
    if (EMBER_SETJMP(ectxB.checkpoint)) b_trapped=true;
